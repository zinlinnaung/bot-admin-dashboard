    ...
    <div className="space-y-6 pb-16">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Customer အားလုံးထံ HTML message နဲ့ animated custom emoji များ
            ပို့နိုင်ပါသည်။
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-black text-emerald-700">
          <Users size={15} /> All active customers
        </div>
      </header>
      ...
      {result && (
        <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
          ...
          <div>
            <p className="font-black">Broadcast စတင်ပြီးပါပြီ</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
        <section className="space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-slate-900">Message Composer</h2>
              <p className="text-xs text-slate-400">
                Telegram HTML formatting supported
              </p>
            </div>
          </div>
            ...
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h3 className="flex items-center gap-2 text-sm font-black text-slate-800">
                  <SmilePlus size={17} className="text-violet-600" /> Animated
                  Emoji Keyboard
                </h3>
                <p className="mt-1 text-[11px] text-slate-500">
                  Admin က Bot ထဲသို့ custom emoji ပါသော message ပို့လျှင် ဒီ
                  keyboard ထဲ အလိုအလျောက်ပေါ်ပါမယ်။
                </p>
              </div>
              <button
                onClick={fetchEmojis}
              ...
              >
                <RefreshCw
                  size={17}
                  className={emojiLoading ? "animate-spin" : ""}
                />
              </button>
            </div>
              ...
              {emojis.map((emoji) => (
                <div key={emoji.id} className="group relative">
                  <button
                    ...
                    title={`${emoji.label || "Custom Emoji"} • ${emoji.customEmojiId}`}
                    className="grid h-11 min-w-11 place-items-center rounded-xl bg-slate-100 px-2 text-xl transition hover:bg-violet-100 hover:scale-105"
                  >
                    {emoji.fallback || "✨"}
                  </button>
                  <button
                    ...
                    className="absolute -right-1 -top-1 hidden h-5 w-5 place-items-center rounded-full bg-rose-500 text-white shadow group-hover:grid"
                  >
                    <Trash2 size={11} />
                  </button>
                </div>
              ))}
              {!emojiLoading && !emojis.length && (
                <p className="self-center px-2 text-xs text-slate-400">
                  မှတ်ထားသော custom emoji မရှိသေးပါ။
                </p>
              )}
            </div>
            <details className="mt-3">
              <summary className="cursor-pointer text-xs font-bold text-indigo-600">
                Emoji ID ကို manually ထည့်မည်
              </summary>
              <div className="mt-3 grid gap-2 sm:grid-cols-[90px_1fr_1fr_auto]">
                <input
                  value={newEmoji.fallback}
                  onChange={(e) =>
                    setNewEmoji({ ...newEmoji, fallback: e.target.value })
                  }
                  maxLength={8}
                  placeholder="✨"
                ...
                />
                <input
                  value={newEmoji.customEmojiId}
                  onChange={(e) =>
                    setNewEmoji({ ...newEmoji, customEmojiId: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEmoji({ ...newEmoji, label: e.target.value })
                  }
                ...
                <button
                  type="button"
                  onClick={saveEmoji}
                  className="grid place-items-center rounded-xl bg-indigo-600 px-4 text-white"
                >
                  <Plus size={18} />
                </button>
              </div>
          ...
          <form onSubmit={handleSendBroadcast} className="space-y-5">
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <span
                  className={`text-xs font-bold ${message.length > 4096 ? "text-rose-600" : "text-slate-400"}`}
                >
                  {message.length}/4096
                </span>
              </div>
              ...
              <textarea
                ...
                placeholder="Customer များထံပို့မည့် message…"
                className="w-full resize-y rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-500/5"
              />
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-xs font-black uppercase tracking-wider text-slate-500">
                    {isCallback ? "Callback Data" : "Button URL"}
                  </label>
                  <button
                    type="button"
                    onClick={() => setIsCallback(!isCallback)}
                    className={`flex items-center gap-1 rounded-full px-2 py-1 text-[9px] font-black ${isCallback ? "bg-amber-500 text-white" : "bg-slate-100 text-slate-500"}`}
                  >
                    <Zap size={10} /> Callback
                  </button>
                </div>
                <input
                  value={btnUrl}
                  onChange={(e) => setBtnUrl(e.target.value)}
                  placeholder={isCallback ? "shop_main" : "https://t.me/…"}
                  className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm"
                />
              </div>
            </div>
            ...
            <div className="flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-800">
              ...
              <p>
                Send နှိပ်ပြီးပါက customer အားလုံးထံ background worker ဖြင့်
                ပို့ပါမည်။ Message နှင့် button ကို အရင် Preview စစ်ပါ။
              </p>
            </div>
            <button
              type="submit"
              disabled={isSending || !message.trim() || message.length > 4096}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 py-4 text-sm font-black text-white shadow-xl shadow-indigo-100 transition hover:bg-indigo-700 disabled:opacity-50"
            >
              {isSending ? (
        ...
        <aside className="h-fit rounded-3xl bg-slate-950 p-5 text-white shadow-2xl sm:p-7 xl:sticky xl:top-24">
          ...
          <div className="mt-6 min-h-52 rounded-3xl bg-[#dfe7ed] p-4">
            <div className="max-w-[92%] rounded-2xl rounded-tl-md bg-white p-4 text-sm leading-6 text-slate-800 shadow-md">
              <p className="whitespace-pre-wrap break-words">
                {renderedPreview || (
                  <span className="italic text-slate-400">
                    Message preview ဒီမှာပေါ်ပါမယ်…
                  </span>
                )}
              </p>
              {btnText && (
                <div className="mt-3 border-t border-slate-100 pt-2 text-center text-xs font-black text-[#2481cc]">
                  {btnText}
                </div>
              )}
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4">
            <h3 className="text-xs font-black text-cyan-300">
              Custom emoji preview အကြောင်း
            </h3>
            <p className="mt-2 text-xs leading-5 text-slate-400">
              Web preview တွင် fallback emoji ကိုပြပါမယ်။ Telegram
              သို့ရောက်တဲ့အခါ Premium animated emoji အဖြစ် လှုပ်ရှားပြသပါမယ်။
            </p>
          </div>
        </aside>
      </div>
