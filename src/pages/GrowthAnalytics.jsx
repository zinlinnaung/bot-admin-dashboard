...
export default function GrowthAnalytics() {
  ...
  const [campaignName, setCampaignName] = useState("facebook");
  const load = async () => {
  ...
  const cleanCampaign = campaignName.toLowerCase().trim().replace(/[^a-z0-9_-]+/g, "_").slice(0, 50);
  const campaignLink = `https://t.me/trustvpn_digital_bot?start=src_${cleanCampaign || "campaign"}`;
  ...
  return <div className="space-y-8">
    ...
    <section className="rounded-xl bg-white p-5 shadow"><h2 className="text-xl font-bold">Campaign Tracking Link ထုတ်ရန်</h2><p className="mt-1 text-sm text-gray-500">Facebook၊ TikTok၊ Telegram post သို့မဟုတ် promotion တစ်ခုချင်းစီအတွက် နာမည်မတူအောင်သုံးပါ။</p><div className="mt-4 flex flex-col gap-3 md:flex-row"><input value={campaignName} onChange={(event) => setCampaignName(event.target.value)} className="flex-1 rounded-xl border px-4 py-3" placeholder="ဥပမာ facebook_august" /><button onClick={() => navigator.clipboard.writeText(campaignLink)} className="rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white">Tracking Link Copy</button></div><code className="mt-3 block break-all rounded-xl bg-gray-100 p-3 text-sm">{campaignLink}</code></section>
